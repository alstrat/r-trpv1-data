...
get_first_data_length <- function(data_sheet){
    ...
    first_blank_index <- min(which((first_row == "") | is.na(first_row)))
    options(warn=0)
...
process_trpv1_input_file <- function(in_file_name, draw_plot_flag) {
    ...
    file_name <- paste(EXPERIMENT_FILES_DIR, "/", in_file_name, sep="")
    out_base_name <- paste(OUTPUT_FILES_DIR, "/", (sub(".xlsx", "", in_file_name)), sep="")
    ...
    input_data_list <- list(aktph, trpv1)
    names(input_data_list) <- DATA_TYPE_HEADERS[AKTPH_INDEX: TRPV_INDEX]
...
plot_data <- function(data, title) {
    ...
    data_plot <- ggplot(d, aes(Var1,value, col=Var2))+ 
        ...
        coord_cartesian(xlim=c(0, 140), ylim=c(-1, 3))+
        scale_x_continuous(breaks=seq(0, 140, 20))+
        scale_y_continuous(breaks=seq(-1, 3, 0.2))
    return(data_plot)
}

plot_data_list <- function(data_list, file_name){
    data_names <- names(data_list)
    plots <- lapply(data_names, function(x) plot_data(data_list[x], x))
...
group_data <- function(data_list, data_file, tab_num, grouping_function) {

    file_base <- sub(".xlsx", "", data_file)
    
    data <- data_list[[tab_num]]
...
dump_into_file <- function(data, file) {
    ...
    lapply(data_names, function(x) 
        ...
        # write.xlsx(data[[x]], file=output_file_name, sheetName=x, append=TRUE))
    
    logVar("    [END] Dumping results into", output_file_name)
}
    
# Group cleansed data from step 1 related to baseline
grouping <- function(data_list,  data_file, draw_plot_flag){

    out_base_name <- paste(OUTPUT_FILES_DIR, "/", sub(".xlsx", "", data_file), "_GRP", sep="")
    grouped_aktph <- group_data(data_list, data_file, AKTPH_INDEX, get_data_group_aktph)
    grouped_trpv <- group_data(data_list, data_file, TRPV_INDEX, get_data_group_trpv)
    ...
    if (draw_plot_flag == 1 ) {
        plot_data_list(grouped_data, out_base_name)
    }
    if (DUMP_INTO_XLSX == 1 ) {
...
trpv1_file <- function(experiment_file){
    ...
    data_list <- process_trpv1_input_file(experiment_file, DRAW_PLOTS)
    grouped_data <- grouping(data_list, experiment_file, DRAW_PLOTS)
    return(grouped_data)
}
...
build_summary_max_tab <- function(aktph, trpv) {

    t_aktph <- t(aktph)
    t_aktph_test_max <- apply(t_aktph, 1, function(x) max(x[TEST_DATA_MIN: TEST_DATA_MAX]))
    t_aktph_group <- vapply(rownames(t_aktph), function(x) gsub( "\\..*$", "", x ), "")
    res <- cbind(t_aktph_test_max, t_aktph_group)
    t_trpv <- t(trpv)
    t_trpv_max <- apply(t_trpv, 1, function(x) max(x[-(1:TEST_DATA_MIN)]))
    t_trpv_group <- vapply(rownames(t_trpv), function(x) gsub( "\\..*$", "", x ), "")
    res <- cbind(res, t_trpv_max)
    res <- cbind(res, t_trpv_group)
    return(res)
}
...
trpv1_files <- function(){
    ...
    summary_file_basename <- paste(OUTPUT_FILES_DIR, "/", "trvp1_aggregated", sep="")
    ...
    for (grouped_data in grouped_data_list[-1]){
        for (name in names(summary) ){
            ...
            if (ncol(grouped_data_matrix) > 1 )  {
                sum_rows <- nrow(summ_data_matrix)
                g_rows <-  nrow(grouped_data_matrix)
                if (sum_rows < 1) {
                    m <- grouped_data_matrix
                }else if (g_rows < 1) {
                    m <- summ_data_matrix
                } else {
                    min_nrow <- min(sum_rows, g_rows)
    ...
    if (DRAW_PLOTS == 1 ) {
        plot_data_list(summary, summary_file_basename)
    }

    summary_max_tab <- build_summary_max_tab(summary[[AKTPH_INDEX]], summary[[TRPV_INDEX]])
    summary[["SUMMARY"]] <- summary_max_tab
    dump_into_file(summary, summary_file_basename)
    return(summary_max_tab)
}


################################# test run-trvp1.R #########################################
# summary <- trpv1_files()
# x <- as.numeric(summary[,1])
# y <- as.numeric(summary[,3])

# aov.trpv1 <- aov(y~x)
# means <- (model.tables(aov.trpv1,"means"))$tables$x
# a <- cbind(matrix(x), matrix(y), matrix(means))
# lm.trpv1 <- lm(y~x)
# a <- cbind(a, predict(lm.trpv1))
